...
:cli-help
    ...
    echo.
    echo Command:
    echo      app               Startup and into react SDK to control workspace.
    echo      dev               Startup and into react SDK with repo directory.
    echo      build             Startup react SDK with repo directory and building it.
    echo.
...
:cli-app
    ...
    echo ^> Startup service
    docker rm -f %PROJECT_NAME%-app
    docker run -ti --rm ^
        -v %CLI_DIRECTORY%\repo\%1:/repo ^
        -w "/repo" ^
        --name %PROJECT_NAME%-app ^
        react.sdk:%PROJECT_NAME% bash
    goto end
